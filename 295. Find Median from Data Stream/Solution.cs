public class MedianFinder {
    PriorityQueue<double> low;
    PriorityQueue<double> high;
    public MedianFinder() {
        high = new PriorityQueue<double>();
        low = new PriorityQueue<double>((double a, double b) => b.CompareTo(a));
    }
    ...
    public void AddNum(int num) {
        ...
        if(low.Count > 0 && high.Count > 0 && low.Peek() > high.Peek())
        {
            double lowVal = low.Poll();
            double highVal = high.Poll();
            high.Add(lowVal);
            low.Add(highVal);
        }
    }
